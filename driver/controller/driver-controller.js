...
/**
 * @function<b>register a driver</b>
...
/**
 * @function<b>generate token for driver</b>
...
const currentBooking = (req, res) => {
  ...
  let driver_id
  let valid=auth.authenticate(req,res,token, "nodejsexpress")
    if(valid) {
      driver_id = req.decoded.driver_id;
...
const pastBooking = (req, res) => {
  ...
  let valid=auth.authenticate(req,res,details.token, "nodejsexpress")
    if(valid){
      details.driver_id = req.decoded.driver_id;
